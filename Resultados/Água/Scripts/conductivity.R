...
example_functions(y = condutividade, x = urb, use_defaut_start = TRUE,
                              start_linear = list(a = 0, b = 7), 
                              start_exp_pos = list(a = 0, b = 50, c = 0.02),
                              start_exp_neg = list(a = 1000, b= -1000, c = -0.02),
...
fit_condutividade <- fit_functions(y = condutividade, x = urb, use_defaut_start = FALSE, confint = TRUE,
                                   start_linear = list(a = 0, b = 7), 
...
fit_condutividade$confint_mean
fit_condutividade$coefs_mean


#png("Conductivity_values_optmized.png", width = 10, height = 9, units = "cm", pointsize = 8, res = 600)

example_functions(y = condutividade, x = urb,use_defaut_start = FALSE,
...
fit_condutividade_stand$aictab

fit_condutividade$aictab


coef_stand <- coef(fit_condutividade_stand$all_models$`Asymetric_Bell-Shaped`)
coef <- coef(fit_condutividade$all_models$`Asymetric_Bell-Shaped`)
...
example_functions(y = condutividade_stand, x = urb,
                  start_linear = as.list(coef(fit_condutividade_stand$all_models$Linear)), 
                  start_exp_pos = as.list(coef(fit_condutividade_stand$all_models$Positive_Exponential)),
                  start_exp_neg = as.list(coef(fit_condutividade_stand$all_models$Negative_Exponential)),
                  start_hof = as.list(coef(fit_condutividade_stand$all_models$`HOF_(Sigmoid)`)),
                  #start_sech_sym = as.list(coef(fit_condutividade_stand$all_models$`Symetric_Bell-Shaped`)),
                  start_sech_asy = as.list(coef(fit_condutividade_stand$all_models$`Asymetric_Bell-Shaped`)))
...
example_functions(y = condutividade_stand, x = urb,
                  ...
                  start_hof = as.list(coef(fit_condutividade_stand$all_models$`HOF_(Sigmoid)`)),
                  #start_sech_sym = as.list(coef(fit_condutividade_stand$all_models$`Symetric_Bell-Shaped`)),
                  start_sech_asy = as.list(c(a = 0, H = exp(1.056614)) ))


coef(fit_condutividade_stand$all_models$`HOF_(Sigmoid)`)
coef(fit_condutividade$all_models$`HOF_(Sigmoid)`)


best_model <- fit_condutividade_stand$best_model$Negative_Exponential
