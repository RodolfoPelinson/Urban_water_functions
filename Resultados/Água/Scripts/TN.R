TN <- water_quality_agua_mean$TN
urb <- water_quality_agua_mean$X.urb
TN_weights <- weights_agua$turbidity_.NTU.
...
fit_TN <- fit_functions(y = TN, x = urb,
                        start_linear = list(a = 0, b = 0.35), 
                        start_exp_pos = list(a = -3, b = 6, c = 0.02),
                        start_exp_neg = list(a = 30, b= -30, c = -0.03),
                        start_hof = list(a = 0, H = 25, m = 30, w = 0.15),
...
example_functions(y = TN, x = urb,
                  start_linear = as.list(coef(fit_TN$all_models$Linear)), 
                  start_exp_pos = as.list(coef(fit_TN$all_models$Positive_Exponential)),
                  start_exp_neg = as.list(coef(fit_TN$all_models$Negative_Exponential)),
                  start_hof = as.list(coef(fit_TN$all_models$`HOF_(Sigmoid)`)),
                  #start_sech_sym = as.list(coef(fit_TN$all_models$`SymetrTN_Bell-Shaped`)),
                  start_sech_asy = as.list(coef(fit_TN$all_models$`Asymetric_Bell-Shaped`)))
...
resultado_TN <- run_all_models(fit_TN, y = TN,x = urb)

list_loglik <- resultado_TN$log_likehoods

aictab <- AICctab(list_loglik, base = TRUE)

write.csv(aictab, "Resultados/Água/Seleção_de_Modelos/Result_TN.csv")
...
coefs_sd <- coef(resultado_TN$all_sd_models$Negative_Exponential$best_model$Quadratic)

new_x <- seq(0,100,0.1)

predicted_mean <- fit_TN$all_predicted$Negative_Exponential
...
coefs_sd2 <- coef(resultado_TN$all_sd_models$`HOF_(Sigmoid)`$all_models$Quadratic)

new_x2 <- seq(0,100,0.1)

predicted_mean2 <- fit_TN$all_predicted$`HOF_(Sigmoid)`
...
png("resultados/Água/Curvas/TN.png", width = 10, height = 9, units = "cm", pointsize = 10, res = 600)

par(mar = c(3.5,4,1,0.1), bty = "l", mfrow = c(1,1))

plot(TN ~ urb , type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "")

title(xlab = "Urban cover (%)", line = 2, cex.lab = 1.25)
title(ylab = "TN", line = 2, cex.lab = 1.25)
