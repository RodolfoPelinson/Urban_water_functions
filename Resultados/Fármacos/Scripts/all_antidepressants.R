...
all <- data_farmacos_by_class$all
urb <- data_farmacos_by_class$urb[is.na(all) == FALSE]
all <- all[is.na(all) == FALSE]
...
fit_all <- fit_functions(y = all, x = urb,
                              start_linear = list(a = 0, b = 0.2), 
                              start_exp_pos = list(a = 0, b = 2, c = 0.03),
                              start_exp_neg = list(a = 30, b= -30, c = -0.01),
                              start_hof = list(a = 0, H = 15, m = 20, w = 0.25),
...
#example_sd(fit_all$best_model[[1]], urb,
...
#max_intercep <- 2*sd(resid(fit_all$best_model[[1]]))

resultado_all <- run_all_models(fit_all, y = all,x = urb) #max_intercept_quad = max_intercep, max_intercept_lin_neg = max_intercep)

list_loglik <- resultado_all$log_likehoods

aictab <- AICctab(list_loglik, base = TRUE)
aictab

write.csv(aictab, "Resultados/Fármacos/Seleção_de_Modelos/Result_all.csv")
...
coefs_sd <- coef(resultado_all$all_sd_models$Linear$all_models$Quadratic)

new_x <- seq(0,100,0.1)

predicted_mean <- fit_all$all_predicted$Linear
...
#coefs_sd2 <- coef(resultado_all$all_sd_models$Positive_Exponential$all_models$Linear_Positive)

#new_x2 <- seq(0,100,0.1)

#predicted_mean2 <- fit_all$all_predicted$Positive_Exponential
...
#coefs_sd3 <- coef(resultado_all$all_sd_models$Linear$all_models$Quadratic)

#new_x3 <- seq(0,100,0.1)

#predicted_mean3 <- fit_all$all_predicted$Linear
...
png("resultados/Fármacos/Curvas/all.png", width = 10, height = 9, units = "cm", pointsize = 10, res = 600)

par(mar = c(3.5,4,1,0.1), bty = "l", mfrow = c(1,1))

plot(all ~ urb , type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "")

title(xlab = "Urban cover (%)", line = 2, cex.lab = 1.25)
title(ylab = "Quantity of antidepressants", line = 2, cex.lab = 1.25)
