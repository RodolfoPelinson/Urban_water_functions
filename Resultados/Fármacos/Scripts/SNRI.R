library(bbmle)
#instSNRI.packages("bbmle")

SNRI <- data_farmacos_by_class$SNRI
urb <- data_farmacos_by_class$urb[is.na(SNRI) == FALSE]
SNRI <- SNRI[is.na(SNRI) == FALSE]
...
example_functions(y = SNRI, x = urb,
...
fit_SNRI <- fit_functions(y = SNRI, x = urb,
                         start_linear = list(a = 0, b = 0.2), 
                         start_exp_pos = list(a = 0, b = 2, c = 0.03),
                         start_exp_neg = list(a = 30, b= -30, c = -0.01),
                         start_hof = list(a = 0, H = 15, m = 20, w = 0.25),
...
#weights = SNRI_weights)

fit_SNRI$aictab

#png("Conductivity_values_optmized.png", width = 10, height = 9, units = "cm", pointsize = 8, res = 600)

example_functions(y = all, x = urb,
                  start_linear = as.list(coef(fit_all$all_models$Linear)), 
                  start_exp_pos = as.list(coef(fit_all$all_models$Positive_Exponential)),
                  start_exp_neg = as.list(coef(fit_all$all_models$Negative_Exponential)),
                  start_hof = as.list(coef(fit_all$all_models$`HOF_(Sigmoid)`)),
                  #start_sech_sym = as.list(coef(fit_all$all_models$`Symetrall_Bell-Shaped`)),
                  start_sech_asy = as.list(coef(fit_all$all_models$`Asymetric_Bell-Shaped`)))
...
#example_sd(fit_SNRI$best_model[[1]], urb,
...
#max_intercep <- 2*sd(resid(fit_SNRI$best_model[[1]]))

resultado_SNRI <- run_all_models(fit_SNRI, y = SNRI,x = urb) #max_intercept_quad = max_intercep, max_intercept_lin_neg = max_intercep)

list_loglik <- resultado_SNRI$log_likehoods

aictab <- AICctab(list_loglik, base = TRUE)
aictab

write.csv(aictab, "Resultados/Fármacos/Seleção_de_Modelos/Result_SNRI.csv")
...
coefs_sd <- coef(resultado_SNRI$all_sd_models$`HOF_(Sigmoid)`$all_models$Quadratic)

new_x <- seq(0,100,0.1)

predicted_mean <- fit_SNRI$all_predicted$`HOF_(Sigmoid)`
...
#coefs_sd2 <- coef(resultado_SNRI$SNRI_sd_models$Positive_Exponential$SNRI_models$Linear_Positive)

#new_x2 <- seq(0,100,0.1)

#predicted_mean2 <- fit_SNRI$SNRI_predicted$Positive_Exponential
...
#coefs_sd3 <- coef(resultado_SNRI$SNRI_sd_models$Linear$SNRI_models$Quadratic)

#new_x3 <- seq(0,100,0.1)

#predicted_mean3 <- fit_SNRI$SNRI_predicted$Linear
...
png("resultados/Fármacos/Curvas/SNRI.png", width = 10, height = 9, units = "cm", pointsize = 10, res = 600)

par(mar = c(3.5,4,1,0.1), bty = "l", mfrow = c(1,1))

plot(SNRI ~ urb , type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "")

title(xlab = "Urban cover (%)", line = 2, cex.lab = 1.25)
title(ylab = "Quantity of SNRI antidepressants", line = 2, cex.lab = 1.25)
