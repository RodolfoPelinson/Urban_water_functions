Bupropion <- water_quality_farmacos$q_Bupropion
...
example_functions(y = Bupropion, x = urb,
                  ...
                  start_sech_asy = list(a = 1, H = 20, m = 54.5, s = 5,r = 0.1))

#dev.off()

fit_Bupropion <- fit_functions(y = Bupropion, x = urb,
                               start_linear = list(a = 2, b = 0.1), 
                               start_exp_pos = list(a = -3, b = 10, c = 0.01),
                               start_exp_neg = list(a = 30, b= -30, c = -0.01),
                               start_hof = list(a = 0, H = 7, m = 50, w = 0.5),
                               #start_sech_sym = list(a = 0, H = 600, m = 95, s = 50),
                               start_sech_asy = list(a = 1, H = 20, m = 54.5, s = 5,r = 0.1))
#weights = Bupropion_weights)

fit_Bupropion$aictab

#png("Conductivity_values_optmized.png", width = 10, height = 9, units = "cm", pointsize = 8, res = 600)

example_functions(y = Bupropion, x = urb,
                  start_linear = as.list(coef(fit_Bupropion$all_models$Linear)), 
                  start_exp_pos = as.list(coef(fit_Bupropion$all_models$Positive_Exponential)),
                  start_exp_neg = as.list(coef(fit_Bupropion$all_models$Negative_Exponential)),
                  start_hof = as.list(coef(fit_Bupropion$all_models$`HOF_(Sigmoid)`)),
                  #start_sech_sym = as.list(coef(fit_Bupropion$all_models$`SymetrBupropion_Bell-Shaped`)),
                  start_sech_asy = as.list(coef(fit_Bupropion$all_models$`Asymetric_Bell-Shaped`)))
...
resultado_Bupropion <- run_all_models(fit_Bupropion, y = Bupropion,x = urb)

list_loglik <- resultado_Bupropion$log_likehoods

aictab <- AICctab(list_loglik, base = TRUE)

write.csv(aictab, "Resultados/Fármacos/Seleção_de_Modelos/Result_Bupropion.csv")
...
coefs_sd <- coef(resultado_Bupropion$all_sd_models$Negative_Exponential$all_models$Quadratic)

new_x <- seq(0,100,0.1)

predicted_mean <- fit_Bupropion$all_predicted$Negative_Exponential
...
coefs_sd2 <- coef(resultado_Bupropion$all_sd_models$Linear$all_models$Quadratic)

new_x2 <- seq(0,100,0.1)

predicted_mean2 <- fit_Bupropion$all_predicted$Linear
...
png("resultados/Fármacos/Curvas/Bupropion.png", width = 10, height = 9, units = "cm", pointsize = 10, res = 600)

par(mar = c(3.5,4,1,0.1), bty = "l", mfrow = c(1,1))

plot(Bupropion ~ urb , type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "")

title(xlab = "Urban cover (%)", line = 2, cex.lab = 1.25)
title(ylab = "Bupropion (ng/L)", line = 2, cex.lab = 1.25)
