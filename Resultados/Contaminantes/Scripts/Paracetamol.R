...
Paracetamol <- water_quality_contaminantes$Paracetamol
urb <- water_quality_contaminantes$X.urb[is.na(Paracetamol) == FALSE]
Paracetamol <- Paracetamol[is.na(Paracetamol) == FALSE]

length(Paracetamol)

data.frame(urb,Paracetamol )


#png("Conductivity_starting_values.png", width = 10, height = 9, units = "cm", pointsize = 8, res = 600)

example_functions(y = Paracetamol, x = urb,
                  start_linear = list(a = 0, b = 1.5), 
                  start_exp_pos = list(a = 0, b = 1.5, c = 0.05),
                  start_exp_neg = list(a = 100, b= -100, c = -0.05),
                  start_hof = list(a = 0, H = 100, m = 50, w = 0.5),
                  #start_sech_sym = list(a = 0, H = 600, m = 95, s = 50),
                  start_sech_asy = list(a = 0, H = 100, m = 30, s = 5,r = 0.999))

#dev.off()

fit_Paracetamol <- fit_functions(y = Paracetamol, x = urb,
                              start_linear = list(a = 0, b = 1.5), 
                              start_exp_pos = list(a = 0, b = 1.5, c = 0.05),
                              start_exp_neg = list(a = 100, b= -100, c = -0.05),
                              start_hof = list(a = 0, H = 100, m = 50, w = 0.5),
                              #start_sech_sym = list(a = 0, H = 600, m = 95, s = 50),
                              start_sech_asy = list(a = 0, H = 100, m = 30, s = 5,r = 0.999))
#weights = Paracetamol_weights)

fit_Paracetamol$aictab

#png("Conductivity_values_optmized.png", width = 10, height = 9, units = "cm", pointsize = 8, res = 600)

example_functions(y = Paracetamol, x = urb,
                  start_linear = as.list(coef(fit_Paracetamol$all_models$Linear)), 
                  start_exp_pos = as.list(coef(fit_Paracetamol$all_models$Positive_Exponential)),
                  start_exp_neg = as.list(coef(fit_Paracetamol$all_models$Negative_Exponential)),
                  start_hof = as.list(coef(fit_Paracetamol$all_models$`HOF_(Sigmoid)`)),
                  #start_sech_sym = as.list(coef(fit_Paracetamol$all_models$`SymetrParacetamol_Bell-Shaped`)),
                  start_sech_asy = as.list(coef(fit_Paracetamol$all_models$`Asymetric_Bell-Shaped`)))
...
#example_sd(fit_Paracetamol$best_model[[1]], urb,
...
#max_intercep <- 2*sd(resid(fit_Paracetamol$best_model[[1]]))

resultado_Paracetamol <- run_all_models(fit_Paracetamol, y = Paracetamol,x = urb) #max_intercept_quad = max_intercep, max_intercept_lin_neg = max_intercep)

list_loglik <- resultado_Paracetamol$log_likehoods

aictab <- AICctab(list_loglik, base = TRUE)
aictab

write.csv(aictab, "Resultados/Contaminantes/Seleção_de_Modelos/Result_Paracetamol.csv")
...
coefs_sd <- coef(resultado_Paracetamol$all_sd_models$Negative_Exponential$all_models$Quadratic)

new_x <- seq(0,100,0.1)

predicted_mean <- fit_Paracetamol$all_predicted$Negative_Exponential
...
#coefs_sd2 <- coef(resultado_Paracetamol$all_sd_models$Positive_Exponential$all_models$Linear_Positive)

#new_x2 <- seq(0,100,0.1)

#predicted_mean2 <- fit_Paracetamol$all_predicted$Positive_Exponential
...
#coefs_sd3 <- coef(resultado_Paracetamol$all_sd_models$Linear$all_models$Quadratic)

#new_x3 <- seq(0,100,0.1)

#predicted_mean3 <- fit_Paracetamol$all_predicted$Linear
...
png("resultados/Contaminantes/Curvas/Paracetamol.png", width = 10, height = 9, units = "cm", pointsize = 10, res = 600)

par(mar = c(3.5,4,1,0.1), bty = "l", mfrow = c(1,1))

plot(Paracetamol ~ urb , type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "")

title(xlab = "Urban cover (%)", line = 2, cex.lab = 1.25)
title(ylab = "Paracetamol", line = 2, cex.lab = 1.25)
