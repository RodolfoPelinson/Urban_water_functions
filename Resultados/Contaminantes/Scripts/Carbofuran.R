...
Carbofuran <- water_quality_contaminantes$Carbofuran
urb <- water_quality_contaminantes$X.urb[is.na(Carbofuran) == FALSE]
Carbofuran <- Carbofuran[is.na(Carbofuran) == FALSE]

length(Carbofuran)

data.frame(urb,Carbofuran )


#png("Conductivity_starting_values.png", width = 10, height = 9, units = "cm", pointsize = 8, res = 600)

example_functions(y = Carbofuran, x = urb,
...
fit_Carbofuran <- fit_functions(y = Carbofuran, x = urb,
                          start_linear = list(a = 2, b = 2), 
                          start_exp_pos = list(a = 0, b = 1, c = 0.06),
                          start_exp_neg = list(a = 100, b= -100, c = -0.5),
                          start_hof = list(a = 0, H = 100, m = 50, w = 0.5),
                          #start_sech_sym = list(a = 0, H = 600, m = 95, s = 50),
                          start_sech_asy = list(a = 0, H = 100, m = 50, s = 5,r = 0.999))
#weights = Carbofuran_weights)

fit_Carbofuran$aictab

#png("Conductivity_values_optmized.png", width = 10, height = 9, units = "cm", pointsize = 8, res = 600)

example_functions(y = Carbofuran, x = urb,
                  start_linear = as.list(coef(fit_Carbofuran$all_models$Linear)), 
                  start_exp_pos = as.list(coef(fit_Carbofuran$all_models$Positive_Exponential)),
                  start_exp_neg = as.list(coef(fit_Carbofuran$all_models$Negative_Exponential)),
                  start_hof = as.list(coef(fit_Carbofuran$all_models$`HOF_(Sigmoid)`)),
                  #start_sech_sym = as.list(coef(fit_Carbofuran$all_models$`SymetrCarbofuran_Bell-Shaped`)),
                  start_sech_asy = as.list(coef(fit_Carbofuran$all_models$`Asymetric_Bell-Shaped`)))
...
#example_sd(fit_Carbofuran$best_model[[1]], urb,
...
#max_intercep <- 2*sd(resid(fit_Carbofuran$best_model[[1]]))

resultado_Carbofuran <- run_all_models(fit_Carbofuran, y = Carbofuran,x = urb) #max_intercept_quad = max_intercep, max_intercept_lin_neg = max_intercep)

list_loglik <- resultado_Carbofuran$log_likehoods

aictab <- AICctab(list_loglik, base = TRUE)
aictab

write.csv(aictab, "Resultados/Contaminantes/Seleção_de_Modelos/Result_Carbofuran.csv")
...
coefs_sd <- coef(resultado_Carbofuran$all_sd_models$`HOF_(Sigmoid)`$all_models$Quadratic)

new_x <- seq(0,100,0.1)

predicted_mean <- fit_Carbofuran$all_predicted$`HOF_(Sigmoid)`
...
coefs_sd2 <- coef(resultado_Carbofuran$all_sd_models$`Asymetric_Bell-Shaped`$all_models$Linear_Negative)

new_x2 <- seq(0,100,0.1)

predicted_mean2 <- fit_Carbofuran$all_predicted$`HOF_(Sigmoid)`
...
png("resultados/Contaminantes/Curvas/Carbofuran.png", width = 10, height = 9, units = "cm", pointsize = 10, res = 600)

par(mar = c(3.5,4,1,0.1), bty = "l", mfrow = c(1,1))

plot(Carbofuran ~ urb , type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "", ylim = c(0,14))

title(xlab = "Urban cover (%)", line = 2, cex.lab = 1.25)
title(ylab = "Carbofuran", line = 2, cex.lab = 1.25)
