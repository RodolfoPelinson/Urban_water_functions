...
Atrazin <- water_quality_contaminantes$Atrazin
urb <- water_quality_contaminantes$X.urb[is.na(Atrazin) == FALSE]
Atrazin <- Atrazin[is.na(Atrazin) == FALSE]

length(Atrazin)

data.frame(urb,Atrazin )


#png("Conductivity_starting_values.png", width = 10, height = 9, units = "cm", pointsize = 8, res = 600)

example_functions(y = Atrazin, x = urb,
...
fit_Atrazin <- fit_functions(y = Atrazin, x = urb,
                                start_linear = list(a = 2, b = 2), 
                                start_exp_pos = list(a = 0, b = 1, c = 0.06),
                                start_exp_neg = list(a = 100, b= -100, c = -0.5),
                                start_hof = list(a = 0, H = 100, m = 50, w = 0.5),
...
example_functions(y = Atrazin, x = urb,
                  start_linear = as.list(coef(fit_Atrazin$all_models$Linear)), 
                  start_exp_pos = as.list(coef(fit_Atrazin$all_models$Positive_Exponential)),
                  start_exp_neg = as.list(coef(fit_Atrazin$all_models$Negative_Exponential)),
                  start_hof = as.list(coef(fit_Atrazin$all_models$`HOF_(Sigmoid)`)),
                  #start_sech_sym = as.list(coef(fit_Atrazin$all_models$`SymetrAtrazin_Bell-Shaped`)),
                  start_sech_asy = as.list(coef(fit_Atrazin$all_models$`Asymetric_Bell-Shaped`)))
...
#example_sd(fit_Atrazin$best_model[[1]], urb,
...
#max_intercep <- 2*sd(resid(fit_Atrazin$best_model[[1]]))

resultado_Atrazin <- run_all_models(fit_Atrazin, y = Atrazin,x = urb) #max_intercept_quad = max_intercep, max_intercept_lin_neg = max_intercep)

list_loglik <- resultado_Atrazin$log_likehoods

aictab <- AICctab(list_loglik, base = TRUE)
aictab

write.csv(aictab, "Resultados/Contaminantes/Seleção_de_Modelos/Result_Atrazin.csv")
...
coefs_sd <- coef(resultado_Atrazin$all_sd_models$Linear$all_models$Linear_Positive)

new_x <- seq(0,100,0.1)

predicted_mean <- fit_Atrazin$all_predicted$Linear
...
coefs_sd2 <- coef(resultado_Atrazin$all_sd_models$Positive_Exponential$all_models$Linear_Positive)

new_x2 <- seq(0,100,0.1)

predicted_mean2 <- fit_Atrazin$all_predicted$Positive_Exponential
...
coefs_sd3 <- coef(resultado_Atrazin$all_sd_models$Linear$all_models$Quadratic)

new_x3 <- seq(0,100,0.1)

predicted_mean3 <- fit_Atrazin$all_predicted$Linear
...
png("resultados/Contaminantes/Curvas/Atrazin.png", width = 10, height = 9, units = "cm", pointsize = 10, res = 600)

par(mar = c(3.5,4,1,0.1), bty = "l", mfrow = c(1,1))

plot(Atrazin ~ urb , type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "")

title(xlab = "Urban cover (%)", line = 2, cex.lab = 1.25)
title(ylab = "Atrazin", line = 2, cex.lab = 1.25)
